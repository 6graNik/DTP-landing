(function ($) {
  ...
  $(document).ready(function() {
    ...
    $('.callback-form__submit').on('click', function() {
        var emailRegExp = new RegExp('(^(?:[\\w\\._]+)@(?:[\\w\\._]+)\.(?:[a-z]{2,6}\\.?))', 'im');
        var phoneRegExp = new RegExp('(^[+]?\\d+)', 'im');
  ...
  /* cтарый juery слайдер */
   ...
   $('.examples__slider-control').on('click',function(){
   		autoslide = 0;
